...
def bbox_iou(box1, box2):
    ...
    if torch.cuda.is_available():
        inter_area = torch.max(inter_rect_x2.float() - inter_rect_x1.float() + 1, torch.zeros(inter_rect_x2.shape).cuda()) * torch.max(
            inter_rect_y2.float() - inter_rect_y1.float() + 1, torch.zeros(inter_rect_x2.shape).cuda())
    else:
        inter_area = torch.max(inter_rect_x2.float() - inter_rect_x1.float() + 1, torch.zeros(inter_rect_x2.shape)) * torch.max(
            inter_rect_y2.float() - inter_rect_y1.float() + 1, torch.zeros(inter_rect_x2.shape))
